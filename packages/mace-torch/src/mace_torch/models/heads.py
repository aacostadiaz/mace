...
class ObservableHead(nn.Module):
    ...
    def per_layer(
        ...
        Returns:
            One ``[n_atoms, dimension]`` tensor per layer, each atom's value
            already the one its own head reads out.
        """
        ...
        values = []
        for features, readout, gated in zip(
            layers, self.readouts, self._gated, strict=True
        ):
            flat = features.reshape(features.shape[0], -1)[..., self.layout]
            value = readout(flat, node_head) if gated else readout(flat)
            if self.num_heads > 1:
    ...
    def _layer_ops(self, layer: int, head: int):
        ...
        if not self._gated[layer]:
            yield (
                "linear",
                readout,
                _head_indices(self._grouped, None, out_irreps, out_owners, head),
            )
            return
