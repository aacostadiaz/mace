...
- [ ] v1.0.0 tagged and published with DOI (no release candidate), as three distributions: `mace-core`, `mace-torch`, `mace-jax`.
...
| **Console-script ownership collision** — two distributions declaring the same `console_scripts` is undefined in pip. | Exactly one distribution provides each `mace_*` entry point: `console_scripts` are removed from the root `setup.cfg`, `mace-launcher` is the sole owner for the coexistence window, and `packages/mace-torch` ships under a distinct distribution name while keeping the `mace_torch` import name. RET-6 deletes the launcher and moves the script table into `mace-torch`, so ownership stays single and `mace-launcher` never reaches PyPI. Asserted in CI. |
...
2. **`MACEBackbone` "no gradients" vs stress-via-strain.** The strain displacement is injected *before* edge vectors are computed (`prepare_graph`), so the derivative engine is not purely downstream of the backbone. ARCH-4 carries a design note reviewed before implementation.
...
10. **v1 naming.** The import names `mace_core`/`mace_torch`/`mace_jax` never collide with `mace` by construction; the distribution name for the new torch package is distinct (§7, console-script row). v1.0.0 publishes those three distributions and only those: `mace-launcher` is coexistence scaffolding, is deleted at RET-6, and is never published. Decided and not revisited.
